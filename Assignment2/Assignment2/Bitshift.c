...
8. Set a bit, toggle the byte, set a bit
...
unsigned char answer_2(unsigned char arg0) {
	//Return a byte with the corresponding bit cleared, and all other bits set.
	return ~(1 << arg0);
}

unsigned char answer_3(unsigned char arg0, int arg1) {
	//Return the same byte with the corresponding bit set, and all other bits unchanged
	return arg0 | (1 << arg1);
}

unsigned char answer_4(unsigned char arg0, int arg1) {
	//Return the same byte with the corresponding bit cleared, and all other bits unchanged.
	return arg0 & ~(1 << arg1);
}

unsigned char answer_5(unsigned char arg0) {
	//Return the byte with all 8 bits toggled.
	return ~arg0;
}

unsigned char answer_6(unsigned char arg0, int arg1) {
	//Return the same byte with the corresponding bit toggled, and all other bits unchanged.
	unsigned char shifted = (1 << arg1);
	return arg0 ^ shifted; //or arg0 ^ (1 << arg1)
}

unsigned char answer_7(unsigned char arg0, int arg1) {
	//Return the value (set or unset) of the corresponding bit.
	return (arg0 >> arg1) & 1;
}

unsigned char answer_8(unsigned char arg0, int arg1, int arg2) {
	//Set the bit corresponding to the first bit number, toggle every bit in the byte, then return the value of the bit corresponding to the second bit number.
	unsigned char toggled = ~(arg0 | (1 << arg1));
	return (toggled >> arg2) & 1;
}

unsigned char answer_9(int arg0) {
	//Return a byte with every bit up to and including that bit number set, with all other bits cleared.
	return ~(~0 << (arg0 + 1));
}

unsigned char answer_10(unsigned char *arg0) {
	//Return the data in the two most significant bits of that byte.
	return (*arg0 & 0b11000000) >> 6;
}
