...
#define BYTES_PER_WALL_BLOCK 6
#define BITS_WALLS_BLOCK 8
#define TOTAL_PIXEL_WALL_BLOCK 144
#define BYTES_PER_ALIEN_BULLET 3
#define BYTES_PER_PLAYER_BULLET 1

#define NUM_ALIENS 15
#define NUM_WALLS 3
#define MAX_BULLET_COUNT_ALIEN 4
...
float max_adc;
long max_lcd_adc;
uint8_t wallVisible[TOTAL_PIXEL_WALL_BLOCK];
uint8_t wallX[TOTAL_PIXEL_WALL_BLOCK];
uint8_t wallY[TOTAL_PIXEL_WALL_BLOCK];
...
uint8_t move_alien_bullet_wall(Sprite *alien_bullet);
uint8_t update_alien_bullet_1(Sprite *alien, Sprite *alien_bullet, Sprite *player);
uint8_t update_alien_bullet_2(Sprite *alien, Sprite *alien_bullet, Sprite *player);
...
uint8_t move_player_bullet_wall(Sprite *player_bullet);
uint8_t update_player_bullet_1(Sprite *alien, Sprite *player_bullet, Sprite *player);
uint8_t update_player_bullet_2(Sprite *alien, Sprite *player_bullet, Sprite *player);
	...
	while (1) {
		...
		else if (game_level == 2)
		{
			while (1) {
				...
				//Move alien bullet
				//update_alien_bullet_1(alien, alien_bullet, &player);
				update_alien_bullet_2(alien, alien_bullet, &player);
				//Move player bullet
				//update_player_bullet_1(alien, player_bullet, &player);
				update_player_bullet_2(alien, player_bullet, &player);
...
uint8_t move_alien_bullet_wall(Sprite *alien_bullet) {
	if (alien_bullet->is_visible)
	{
		for (int i = 0; i < TOTAL_PIXEL_WALL_BLOCK; i++)
		{
			if (wallVisible[i] == 1)
			{
				if (//If hit wall
					(uint8_t)(alien_bullet->x) == (uint8_t)wallX[i] &&
					(uint8_t)(alien_bullet->y + alien_bullet->height - 1 ) == (uint8_t)wallY[i])
				{
					alien_bullet->is_visible = false;
					alien_bullet->x = -1;
					alien_bullet->y = -1;
...
uint8_t update_alien_bullet_2(Sprite *alien, Sprite *alien_bullet, Sprite *player) {
	...
	if (alien_bullet_timer == 0)
	{
		...
		{
			changed = move_alien_bullet(&alien_bullet[i], player);
			move_alien_bullet_wall(&alien_bullet[i]);
			move_alien_bullet_wall(&alien_bullet[i]);
			move_alien_bullet_wall(&alien_bullet[i]);
		}
	}
...
uint8_t move_player_bullet_wall(Sprite *player_bullet) {
	if (player_bullet->is_visible)
	{
		for (int i = 0; i < TOTAL_PIXEL_WALL_BLOCK; i++)
		{
			...
			{
				if (
					(uint8_t)player_bullet->x == (uint8_t)wallX[i] &&
					(uint8_t)player_bullet->y == (uint8_t)wallY[i])
				{
					player_bullet->is_visible = false;
					player_bullet->x = -1;
					player_bullet->y = -1;
...
uint8_t update_player_bullet_2(Sprite *alien, Sprite *player_bullet, Sprite *player) {
	...
	if (player_bullet_timer == 0)
	{
		...
		{
			changed = move_player_bullet(&player_bullet[i], alien, i);
			move_player_bullet_wall(&player_bullet[i]);
			move_player_bullet_wall(&player_bullet[i]);
			move_player_bullet_wall(&player_bullet[i]);
		}
	}
...
void init_wall_location()
{
	for (unsigned char i = 0; i < TOTAL_PIXEL_WALL_BLOCK; i++) {
		wallVisible[i] = true;
	}

	//Rows of walls
	for (uint8_t column = 0; column < BITS_WALLS_BLOCK; column++)
	{
		set_pixel(WALL1_X + column, WALL_Y, 1);
		set_pixel(WALL2_X + column, WALL_Y, 1);
		set_pixel(WALL3_X + column, WALL_Y, 1);
		wallX[column] = WALL1_X + column;
		wallY[column] = WALL_Y;

		wallX[column + 48] = WALL2_X + column;
		wallY[column + 48] = WALL_Y;

		wallX[column + 96] = WALL3_X + column;
		wallY[column + 96] = WALL_Y;
	}

	for (uint8_t column = BITS_WALLS_BLOCK; column < BITS_WALLS_BLOCK * 2; column++)
	{
		set_pixel(WALL1_X + column - BITS_WALLS_BLOCK, WALL_Y + 1, 1);
		set_pixel(WALL2_X + column - BITS_WALLS_BLOCK, WALL_Y + 1, 1);
		set_pixel(WALL3_X + column - BITS_WALLS_BLOCK, WALL_Y + 1, 1);
		wallX[column] = WALL1_X + column - BITS_WALLS_BLOCK;
		wallY[column] = WALL_Y+1;

		wallX[column + 48] = WALL2_X + column - BITS_WALLS_BLOCK;
		wallY[column + 48] = WALL_Y+1;

		wallX[column + 96] = WALL3_X + column - BITS_WALLS_BLOCK;
		wallY[column + 96] = WALL_Y+1;
	}

	for (uint8_t column = BITS_WALLS_BLOCK * 2; column < BITS_WALLS_BLOCK * 3; column++)
	{
		set_pixel(WALL1_X + column - BITS_WALLS_BLOCK * 2, WALL_Y + 2, 1);
		set_pixel(WALL2_X + column - BITS_WALLS_BLOCK * 2, WALL_Y + 2, 1);
		set_pixel(WALL3_X + column - BITS_WALLS_BLOCK * 2, WALL_Y + 2, 1);
		wallX[column] = WALL1_X + column - BITS_WALLS_BLOCK * 2;
		wallY[column] = WALL_Y + 2;

		wallX[column + 48] = WALL2_X + column - BITS_WALLS_BLOCK * 2;
		wallY[column + 48] = WALL_Y + 2;

		wallX[column + 96] = WALL3_X + column - BITS_WALLS_BLOCK * 2;
		wallY[column + 96] = WALL_Y + 2;
	}

	for (uint8_t column = BITS_WALLS_BLOCK * 3; column < BITS_WALLS_BLOCK * 4; column++)
	{
		set_pixel(WALL1_X + column - BITS_WALLS_BLOCK * 3, WALL_Y + 3, 1);
		set_pixel(WALL2_X + column - BITS_WALLS_BLOCK * 3, WALL_Y + 3, 1);
		set_pixel(WALL3_X + column - BITS_WALLS_BLOCK * 3, WALL_Y + 3, 1);
		wallX[column] = WALL1_X + column - BITS_WALLS_BLOCK * 3;
		wallY[column] = WALL_Y + 3;

		wallX[column + 48] = WALL2_X + column - BITS_WALLS_BLOCK * 3;
		wallY[column + 48] = WALL_Y + 3;

		wallX[column + 96] = WALL3_X + column - BITS_WALLS_BLOCK * 3;
		wallY[column + 96] = WALL_Y + 3;
	}

	for (uint8_t column = BITS_WALLS_BLOCK * 4; column < BITS_WALLS_BLOCK * 5; column++)
	{
		set_pixel(WALL1_X + column - BITS_WALLS_BLOCK * 4, WALL_Y + 4, 1);
		set_pixel(WALL2_X + column - BITS_WALLS_BLOCK * 4, WALL_Y + 4, 1);
		set_pixel(WALL3_X + column - BITS_WALLS_BLOCK * 4, WALL_Y + 4, 1);
		wallX[column] = WALL1_X + column - BITS_WALLS_BLOCK * 4;
		wallY[column] = WALL_Y + 4;

		wallX[column + 48] = WALL2_X + column - BITS_WALLS_BLOCK * 4;
		wallY[column + 48] = WALL_Y + 4;

		wallX[column + 96] = WALL3_X + column - BITS_WALLS_BLOCK * 4;
		wallY[column + 96] = WALL_Y + 4;
	}

	for (uint8_t column = BITS_WALLS_BLOCK * 5; column < BITS_WALLS_BLOCK * 6; column++)
	{
		set_pixel(WALL1_X + column - BITS_WALLS_BLOCK * 5, WALL_Y + 5, 1);
		set_pixel(WALL2_X + column - BITS_WALLS_BLOCK * 5, WALL_Y + 5, 1);
		set_pixel(WALL3_X + column - BITS_WALLS_BLOCK * 5, WALL_Y + 5, 1);
		wallX[column] = WALL1_X + column - BITS_WALLS_BLOCK * 5;
		wallY[column] = WALL_Y + 5;

		wallX[column + 48] = WALL2_X + column - BITS_WALLS_BLOCK * 5;
		wallY[column + 48] = WALL_Y + 5;

		wallX[column + 96] = WALL3_X + column - BITS_WALLS_BLOCK * 5;
		wallY[column + 96] = WALL_Y + 5;
	}
